...
module game{
	export class PageManager{
		...
		{
			this.baseShow("loginView", view.login);
		}

		public showEnter():void
		{
			this.baseShow("enterView", view.enter);
		}

		public showRoom():void
		{
			this.baseShow("roomView", view.room);
		}

		public getRoomView():view.room
		{
			if (this.roomView == null) {
				this.roomView = new view.room();
				Laya.stage.addChild(this.roomView);
				this.roomView.visible = false;
			}

			return (this.roomView as view.room);
		}
	}
}